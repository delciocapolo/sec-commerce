...
function StarRating_Component($filledPercent = 100)
{
   // garante que o valor esteja entre 0 e 100
   $filledPercent = max(0, min(100, $filledPercent));

   // define a largura da parte amarela com base no preenchimento
   $fillStyle = "width: {$filledPercent}%";
   ...
   return <<<HTML
      <span class="relative inline-block w-6 h-6 text-gray-300">
         ...
         <span class="absolute top-0 left-0 h-full overflow-hidden text-yellow-400" style="$fillStyle">
